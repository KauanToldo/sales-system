<script setup>
import { computed, onMounted, ref } from 'vue'
import { PhPencilSimpleLine, PhTrash } from '@phosphor-icons/vue'
import { toast } from 'vue3-toastify'
import http from '../api/http'
import SearchInput from '../components/common/SearchInput.vue'
import PrimaryActionButton from '../components/common/PrimaryActionButton.vue'
import PaginatedDataTable from '../components/common/PaginatedDataTable.vue'
import ConfirmDeleteModal from '../components/products/ConfirmDeleteModal.vue'
import CustomerFormModal from '../components/customers/CustomerFormModal.vue'

const customerColumns = [
  {
    key: 'name',
    label: 'Customer Name',
    cellClass: 'font-semibold',
  },
  {
    key: 'email',
    label: 'Email',
    cellClass: 'text-sales-ink',
  },
  {
    key: 'phone',
    label: 'Phone',
    cellClass: 'text-sales-ink',
  },
]

const customers = ref([])
const isLoading = ref(false)
const search = ref('')

const isFormModalOpen = ref(false)
const isDeleteModalOpen = ref(false)
const isEditing = ref(false)
const isSubmittingForm = ref(false)
const isDeleting = ref(false)
const selectedCustomer = ref(null)

const currentPage = ref(1)
const perPage = 10

const getApiErrorMessage = (error, fallback) => {
  return error?.response?.data?.error || fallback
}

const fetchCustomers = async () => {
  isLoading.value = true

  try {
    const { data } = await http.get('/customers')
    customers.value = Array.isArray(data) ? data : []

    const maxPage = Math.max(1, Math.ceil(customers.value.length / perPage))
    if (currentPage.value > maxPage) {
      currentPage.value = maxPage
    }
  } catch (error) {
    toast.error(getApiErrorMessage(error, 'Unable to load customers'))
  } finally {
    isLoading.value = false
  }
}

const filteredCustomers = computed(() => {
  const term = search.value.trim().toLowerCase()

  if (!term) {
    return customers.value
  }

  return customers.value.filter((customer) => {
    return [customer.name, customer.email, customer.phone]
      .map((value) => String(value || '').toLowerCase())
      .some((value) => value.includes(term))
  })
})

const totalPages = computed(() => {
  return Math.max(1, Math.ceil(filteredCustomers.value.length / perPage))
})

const paginatedCustomers = computed(() => {
  const start = (currentPage.value - 1) * perPage
  return filteredCustomers.value.slice(start, start + perPage)
})

const paginationLabel = computed(() => {
  const total = filteredCustomers.value.length

  if (total === 0) {
    return 'Showing 0 to 0 of 0 entries'
  }

  const start = (currentPage.value - 1) * perPage + 1
  const end = Math.min(currentPage.value * perPage, total)

  return `Showing ${start} to ${end} of ${total} entries`
})

const visiblePages = computed(() => {
  const pages = []
  const start = Math.max(1, currentPage.value - 1)
  const end = Math.min(totalPages.value, start + 2)

  for (let page = start; page <= end; page += 1) {
    pages.push(page)
  }

  return pages
})

const setPage = (page) => {
  if (page < 1 || page > totalPages.value) {
    return
  }

  currentPage.value = page
}

const openCreateModal = () => {
  isEditing.value = false
  selectedCustomer.value = {
    name: '',
    email: '',
    phone: '',
  }
  isFormModalOpen.value = true
}

const openEditModal = (customer) => {
  isEditing.value = true
  selectedCustomer.value = {
    id: customer.id,
    name: customer.name,
    email: customer.email,
    phone: customer.phone,
  }
  isFormModalOpen.value = true
}

const closeFormModal = () => {
  if (isSubmittingForm.value) {
    return
  }

  isFormModalOpen.value = false
}

const handleSubmitCustomer = async (payload) => {
  isSubmittingForm.value = true

  try {
    if (isEditing.value && selectedCustomer.value?.id) {
      await http.put(`/customers/${selectedCustomer.value.id}`, payload)
      toast.success('Customer updated successfully')
    } else {
      await http.post('/customers', payload)
      toast.success('Customer created successfully')
    }

    isFormModalOpen.value = false
    await fetchCustomers()
  } catch (error) {
    toast.error(getApiErrorMessage(error, 'Unable to save customer'))
  } finally {
    isSubmittingForm.value = false
  }
}

const openDeleteModal = (customer) => {
  selectedCustomer.value = customer
  isDeleteModalOpen.value = true
}

const closeDeleteModal = () => {
  if (isDeleting.value) {
    return
  }

  isDeleteModalOpen.value = false
}

const handleDelete = async () => {
  if (!selectedCustomer.value?.id) {
    return
  }

  isDeleting.value = true

  try {
    await http.delete(`/customers/${selectedCustomer.value.id}`)
    toast.success('Customer deleted successfully')
    isDeleteModalOpen.value = false
    await fetchCustomers()
  } catch (error) {
    toast.error(getApiErrorMessage(error, 'Unable to delete customer'))
  } finally {
    isDeleting.value = false
  }
}

onMounted(() => {
  fetchCustomers()
})
</script>

<template>
  <section class="space-y-4">
    <header>
      <h1 class="font-display text-2xl font-semibold text-sales-tertiary">Customer Management</h1>
      <p class="mt-1 text-sm text-sales-ink">Manage customer accounts, contacts, and communication details.</p>
    </header>

    <div class="rounded-xl border border-sales-border bg-sales-surface shadow-sm">
      <div class="flex flex-col gap-3 border-b border-sales-border p-4 md:flex-row md:items-center md:justify-between">
        <div class="w-full md:max-w-sm">
          <SearchInput v-model="search" placeholder="Search customers..." />
        </div>

        <PrimaryActionButton label="Novo Cliente" @click="openCreateModal" />
      </div>

      <PaginatedDataTable
        :columns="customerColumns"
        :rows="paginatedCustomers"
        :loading="isLoading"
        loading-message="Loading customers..."
        empty-message="No customers found."
        :pagination-label="paginationLabel"
        :current-page="currentPage"
        :total-pages="totalPages"
        :visible-pages="visiblePages"
        show-actions
        actions-label="Actions"
        @prev="setPage(currentPage - 1)"
        @next="setPage(currentPage + 1)"
        @page="setPage"
      >
        <template #actions="{ row }">
          <div class="flex items-center gap-2">
            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md border border-sales-border px-2.5 py-1.5 text-xs font-semibold text-sales-tertiary transition hover:bg-sales-muted"
              @click="openEditModal(row)"
            >
              <PhPencilSimpleLine :size="14" />
              Edit
            </button>

            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-100"
              @click="openDeleteModal(row)"
            >
              <PhTrash :size="14" />
              Delete
            </button>
          </div>
        </template>
      </PaginatedDataTable>
    </div>

    <CustomerFormModal
      :open="isFormModalOpen"
      :is-editing="isEditing"
      :initial-data="selectedCustomer || {}"
      :is-submitting="isSubmittingForm"
      @close="closeFormModal"
      @submit="handleSubmitCustomer"
    />

    <ConfirmDeleteModal
      :open="isDeleteModalOpen"
      :is-submitting="isDeleting"
      title="Delete customer"
      :message="`Are you sure you want to delete ${selectedCustomer?.name || 'this customer'}?`"
      @close="closeDeleteModal"
      @confirm="handleDelete"
    />
  </section>
</template>
